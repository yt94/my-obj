...
(function() {
    ...
    var masterProdcutPriceBox = document.querySelector('#masterProductPrice');   // 显示主商品价格的元素
    var checkboxItems = document.querySelectorAll('#chooseProducts input'); // 所有搭配商品的复选框
    var totalNumBox = document.querySelector('#totalNum'); // 商品件数的元素
    ...
    masterProdcutPriceBox.innerHTML = '&yen;' + productPrice;
    ...
    // 给所有的搭配商品的复选框监听 change 事件
    checkboxItems.forEach(function(checkBoxItem) {
        checkBoxItem.addEventListener('change', function() {
            // 如果当前复选框是选中的，说明从不选中变化为选中
            if (checkBoxItem.checked) {
                // 加上当前搭配商品的价格
                collectionPrice += (+checkBoxItem.value);
                // 搭配商品的数量 ++
                collectionNumber ++;
            } else {
                // 当前是不选中，说明从选中到不选中
            ...
            // 显示搭配商品数量
            totalNumBox.innerHTML = collectionNumber;

            // 设置总价 主商品 + 搭配商品
            totalPriceBox.innerHTML = '&yen;' + (productPrice + collectionPrice);
        });
    });
    ...
    function computedPrice() {
        ...
        // 计算主商品总价
        productPrice = price * productNumbers
